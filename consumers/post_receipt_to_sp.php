...
$channel->queue_declare('receipt.to.sp.retry', false, true, false, false, false, [
...
$channel->queue_bind('receipt.to.sp', 'sp_exchange', 'gepg.receipt');
$channel->queue_bind('receipt.to.sp.retry', 'sp_exchange', 'gepg.receipt.retry'); 
 ...
 $cb_bill_out = function($msg) {	
	...
	$response = curlRequest(SP_SERVER.SP_RECEIPT_PATH, null, $msg->body);
    ...
    else 
    {
    	...
    	$msg->delivery_info['channel']->basic_publish($msg,'sp_exchange','gepg.receipt.retry');
		// } else {
...
$channel->basic_consume('receipt.to.sp', '', false, false, false, false, $cb_bill_out);
