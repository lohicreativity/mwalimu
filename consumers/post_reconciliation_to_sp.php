...
$channel->queue_declare('recon.to.sp', false, true, false, false);
$channel->queue_declare('recon.to.sp.retry', false, true, false, false, false, [
...
$channel->queue_bind('recon.to.sp', 'sp_exchange', 'gepg.recon');
$channel->queue_bind('recon.to.sp.retry', 'sp_exchange', 'gepg.recon.retry'); 
 ...
 $cb_bill_out = function($msg) {	
	...
	$response = curlRequest(SP_SERVER.SP_RECON_PATH, null, $msg->body);
	...
	if( $response['code'] == 200 ) {
		...
		echo "\n * RECON => SP Response: ".json_encode($response['body']), "\n";   
    ...
    else 
    {
    	...
    	$msg->delivery_info['channel']->basic_publish($msg,'sp_exchange','gepg.recon.retry');
		// } else {
...
$channel->basic_consume('recon.to.sp', '', false, false, false, false, $cb_bill_out);
