...
->where('campus_id',$request->get('campus_id'))->where('status','ACTIVE')->latest()->first(); */
    ...
    public function editBasicInfo(Request $request)
    {
      ...
      if($medical_insurance_fee &&  $students_union_fee && $caution_money_fee && $medical_examination_fee && $registration_fee && 
         $identity_card_fee && $late_registration_fee && $welfare_emergence_fund &&  $quality_assurance_fee && $reference_number){
    ...
    public function submission(Request $request)
      {
         ...
         $applicants = Applicant::where('program_level_id', $applicant->program_level_id)
                                ->where('submission_complete_status', 1)
                                ->where('application_window_id', $applicant->application_window_id)
                                ->whereNotNull('status')
                                ->where('batch_id',8)
                                ->first();
    ...
    public function editApplicantDetails(Request $request)
      {
         ...
         'awards'=>Award::all(),
		   'countries'=>Country::all(),
         'invoice'=>$request->get('index_number')? Invoice::whereNull('gateway_payment_id')->whereNull('control_no')
                                                          ->where(function($query) use($applicant, $student_id){$query->where('payable_id',$applicant->id)->where('payable_type','applicant')
                                                          ->orWhere(function($q) use($student_id){$q->where('payable_id',$student_id)->where('payable_type','student');});})
                                                          ->first() : null,
         'batch'=> $batch,
