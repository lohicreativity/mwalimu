...
class StaffController extends Controller
{
    ...
    public function viewPayerDetails(Request $request)
    {
		if(!empty($request->keyword)){
			...
			$applicant_payer? $paid_as_applicant = Invoice::where('payable_id',$applicant_payer->id)->with('feeItem.feeType','gateway_payment')->get() : $paid_as_applicant = null;
			count($student_payer)>0 ? $paid_as_student = Invoice::where('payable_id', $student_payer->id)->with('feeItem.feeType','gateway_payment')->get() : $student_payer = null;
