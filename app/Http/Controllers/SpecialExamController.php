    ...
    public function showPostponement(Request $request)
    {
        ...
        $data =  [
           ...
           'opted_module'=>ModuleAssignment::whereHas('programModuleAssignment',function($query) use($student){
            ...
            ->where('study_academic_year_id',session('active_academic_year_id'))
            ->get(), 
            'special_exam_requests'=>SpecialExamRequest::with(['exams.moduleAssignment.programoduleAssignment','exams.moduleAssignment.module'])->where('student_id',$student->id)->paginate(20),
            'student'=>$student,
            'request'=>$request,
            'suppExams'     => $suppExams
        ];
