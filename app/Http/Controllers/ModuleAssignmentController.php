  ...
  public function assignmentConfirmation(Request $request)
  {
    $staff = User::find(Auth::user()->id)->staff()->with(['department'])->first();
      $data = [
           ...
           'study_academic_years'=>StudyAcademicYear::with('academicYear')->get(),
           'assignments'=>ModuleAssignment::whereHas('programModuleAssignment.moduleAssignments.staff',function($query) use ($staff){
                    $query->where('department_id',$staff->department_id);
               })->with(['programModuleAssignment.moduleAssignments.staff','programModuleAssignment.campusProgram.program','studyAcademicYear.academicYear','staff.campus'])->latest()->where('study_academic_year_id',$request->get('study_academic_year_id'))->paginate(20),
    ...
    public function showStaffAssignedModules(Request $request)
    {
        ...
        $data = [
           ...
           'assignments'=>$staff? ModuleAssignment::whereHas('studyAcademicYear',function($query) use ($request){
             ...
             })->with(['studyAcademicYear.academicYear','module','programModuleAssignment.campusProgram.program','programModuleAssignment.campusProgram.campus','programModuleAssignment.semester'])->where('staff_id',$staff->id)->where('confirmed',1)->latest()->paginate(20) : [],
        ];
