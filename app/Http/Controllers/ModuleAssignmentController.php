 ...
 */		   'module_assignment_requests'=>ModuleAssignmentRequest::with(['programModuleAssignment.moduleAssignments.staff','campusProgram.program','studyAcademicYear.academicYear','user.staff.campus'])
  ...
  /**
         ...
         try{
            ...
            $module_assignment = ModuleAssignment::with('assessmentPlans','module','programModuleAssignment')->findOrFail($id);
             ...
             $students_with_final_marks_count = ExaminationResult::whereHas('student.studentshipStatus',function($query){
                    $query->where('name','ACTIVE')->orWhere('name','RESUMED');
                })->where('module_assignment_id',$module_assignment->id)->where('exam_type','FINAL')->whereNotNull('final_score')->whereNotNull('final_uploaded_at')->count();
    ...
    public function postponedStudents(Request $request,$id)
    {
        try{
           ...
           $data = [
            ...
            return view('dashboard.academic.reports.postponed-students',$data);
        ...
        try{
           ...
           $data = [
                ...
                'results'=>ExaminationResult::whereHas('student.studentshipStatus',function($query){
                    $query->where('name','ACTIVE')->OrWhere('name','RESUMED');
                })->with('student')->where('module_assignment_id',$module_assignment->id)->where('exam_type','FINAL')->whereNotNull('final_score')->whereNotNull('final_uploaded_at')->get(),
				'semester'=>$module_assignment->programModuleAssignment->semester_id
            ];
    ...
    {
        try{
           $module_assignment = ModuleAssignment::with(['programModuleAssignment.campusProgram.program.departments',
                                                        'programModuleAssignment.campusProgram.campus',
                                                        'programModuleAssignment.campusProgram.program',
                                                        'studyAcademicYear.academicYear',
                                                        'programModuleAssignment.module',
                                                        'programModuleAssignment.students',
                                                        'module'])
                                                ->findOrFail($id);
            foreach($module_assignment->programModuleAssignment->campusProgram->program->departments as $dpt){
                if($dpt->pivot->campus_id == $module_assignment->programModuleAssignment->campusProgram->campus_id){
            ...
            if(ResultPublication::whereHas('semester',function($query) use($semester){$query->where('id',$semester->id);})
                                ->where('study_academic_year_id',$module_assignment->study_academic_year_id)
                                ->where('campus_id',$module_assignment->programModuleAssignment->campusProgram->campus->id)
                                ->where('type','SUPP')
                                ->where('nta_level_id',$module_assignment->programModuleAssignment->campusProgram->program->nta_level_id)
                                ->where('status','PUBLISHED')->count() != 0){
                $supp_published = true;
            ...
            $data = [
                ...
                'results'=>ExaminationResult::whereHas('student.studentshipStatus',function($query){$query->where('name','ACTIVE')->OrWhere('name','RESUMED');})
                                            ->whereHas('student.registrations',function($query) use($module_assignment){$query->where('year_of_study',$module_assignment->programModuleAssignment->year_of_study)
                                                                                                                              ->where('semester_id',$module_assignment->programModuleAssignment->semester_id)
                                                                                                                              ->where('study_academic_year_id',$module_assignment->study_academic_year_id);})
                                            ->whereDoesntHave('retakeHistory',function($query) use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'));})
    ...
    {  
        ...
        if($request->hasFile('results_file')){
            $module_assignment = ModuleAssignment::with(['module','studyAcademicYear.academicYear',
                                                         'programModuleAssignment.campusProgram',
                                                         'programModuleAssignment.campusProgram.program'])
                                                 ->find($request->get('module_assignment_id'));

            if($request->get('assessment_plan_id') != 'SUPPLEMENTARY'){
                
            //     $supp_published = false;
            //     if(ResultPublication::whereHas('semester',function($query) use($module_assignment){$query->where('id',$module_assignment->programModuleAssignment->semester_id);})
            //                         ->where('study_academic_year_id',$module_assignment->study_academic_year_id)
            //                         ->where('campus_id',$module_assignment->programModuleAssignment->campusProgram->campus_id)
            //                         ->where('type','SUPP')
            //                         ->where('nta_level_id',$module_assignment->programModuleAssignment->campusProgram->program->nta_level_id)
            ...
            //         $special_exam = SpecialExam::whereHas('student.campusProgram',function($query) use($module_assignment){$query->where('campus_id',$module_assignment->campus_id)
            //                                                                                                                      ->where('id',$module_assignment->programModuleAssignment->campus_program_id);})
            //                                    ->whereHas('student',function($query) use($module_assignment){$query->where('year_of_study',$module_assignment->programModuleAssignment->year_of_study)
            //                                                                                                        ->where('registration_year',explode('/',$module_assignment->studyAcademicYear->academicYear->year)[0]);})
            //                                    ->whereHas('student.academicStatus',function($query){$query->where('name','PASS')
            //                                                                                               ->orWhere('name','FRESHER')
            //                                                                                               ->orWhere('name','CARRY')
            //                                                                                               ->orWhere('name','RETAKE')
            //                                                                                               ->orWhere('name','POSTPONED')
            //                                                                                               ->orWhere('name','SUPP');})
            //                                    ->where('study_academic_year_id',$module_assignment->programModuleAssignment->study_academic_year_id)
            //                                    ->where('semester_id',$module_assignment->programModuleAssignment->semester_id)
            //                                    ->where('type','SUPP')
            ...
            //             $all_students[] = $special_exam->student;
            //         }
            ...
            if($module_assignment->programModuleAssignment->category == 'OPTIONAL'){
                if($request->get('assessment_plan_id') != 'SUPPLEMENTARY'){
                    ...
                    $non_opted_students = $invalid_students = $invalid_carry_students = [];
                    foreach($uploaded_students as $up_stud){
                        if($module_assignment->programModuleAssignment->students()->whereHas('academicStatus',function($query){$query->where('name','POSTPONED')
                                                                                                                                     ->orWhere('name','SUPP')
                                                                                                                                     ->orWhere('name','INCOMPLETE')
                                                                                                                                     ->orWhere('name','CARRY')
                                                                                                                                     ->orWhere('name','RETAKE');})
                                                                                  ->where('id',$up_stud->id)
                                                                                  ->count() == 0){
                            ...
                            if($supp_published){
                                if(ExaminationResult::where('module_assignment_id',$module_assignment->id)
                                                    ->where('student_id',$up_stud->id)
                                                    ->where('course_work_remark','!=','FAIL')
                                                    ->whereNotNull('supp_uploaded_at')
                                                    ->where('final_remark','POSTPONED')
                                                    ->where('final_exam_remark','FAIL')
                ...
                foreach($uploaded_students as $up_stud){
                    if($request->get('assessment_plan_id') == 'SUPPLEMENTARY'){
                        if($supp_published){
                            if(ExaminationResult::where('student_id',$up_stud->id)
                                                ->where('module_assignment_id',$module_assignment->id)
                                                ->whereNotNull('supp_uploaded_at')
                                                ->where('final_remark','POSTPONED')
                                                ->where('final_exam_remark','FAIL')
                        ...
                        $carry_case = ExaminationResult::select('student_id')
                                                       ->whereHas('moduleAssignment.programModuleAssignment',function($query) use($module_assignment){$query->where('campus_program_id',$module_assignment->programModuleAssignment->campusProgram->id)
                                                                                                                                                             ->where('year_of_study',1)
                                                                                                                                                             ->where('semester_id',$module_assignment->programModuleAssignment->semester_id)
                                                                                                                                                             ->where('study_academic_year_id',$module_assignment->programModuleAssignment->study_academic_year_id -1 );})
                                                        ->whereHas('student',function($query) use($module_assignment,$up_stud){$query->where('campus_program_id',$module_assignment->programModuleAssignment->campusProgram->id)
                                                                                                                                     ->where('id',$up_stud->id);})
                                                        //->whereHas('student.applicant',function($query) use($request){$query->where('intake_id',$request->get('intake_id'));}) // NEED TO CROSSCHECK THIS
                                                        ->whereHas('student.studentshipStatus',function($query){$query->where('name','ACTIVE')->orWhere('name','RESUMED');})
                                                        ->whereHas('student.registrations',function($query) use($module_assignment){$query->where('year_of_study',2)
                                                                                                                                          ->where('semester_id',$module_assignment->programModuleAssignment->semester_id)
                                                                                                                                          ->where('study_academic_year_id',$module_assignment->programModuleAssignment->study_academic_year_id);})
                        ...
                        if($student){
                            ...
                            if($carry_case){
                                $students[] = Student::select('id','academic_status_id','year_of_study')->where('id',$carry_case->student_id)->with('academicStatus:id,name')->first();
                            }else{
            ...
            foreach($missing_students as $student){
                if($request->get('assessment_plan_id') == 'FINAL_EXAM'){
                    ...
                    if($semester_remark){
                        if($semester_remark->remark == 'SUPP' || $sup_special_exam || $final_special_exam){
                            $supp_upload_allowed = true;
                        }else{
                            if(($semester_remark->remark == 'CARRY' && $student->year_of_study == 1 || $semester_remark->remark == 'RETAKE' || $semester_remark->remark == 'INCOMPLETE') &&
                                ExaminationResult::where('module_assignment_id',$module_assignment->id)->where('student_id',$student->id)->where('course_work_remark','FAIL')->count() == 0){
                                $supp_upload_allowed = true;
