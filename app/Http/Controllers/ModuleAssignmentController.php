 ...
 */		   'module_assignment_requests'=>ModuleAssignmentRequest::with(['programModuleAssignment.moduleAssignments.staff','campusProgram.program','studyAcademicYear.academicYear','user.staff.campus'])
     ...
     //               $query->where('d',$staff->department_id);
       //        })->
       $x = Staff::whereHas('department.campuses')->with(['designation','campus','department'])->get();
return count($x);
    ...
    public function showResultsUpload(Request $request,$id)
    {
         try{
             ...
             $supp_cases_count = ExaminationResult::whereHas('student.studentshipStatus',function($query){
                    $query->where('name','ACTIVE')->orWhere('name','RESUMED');
                })->whereHas('student.annualRemarks', function($query){$query->where('remark','SUPP');})->where('module_assignment_id',$module_assignment->id)->whereNotNull('final_uploaded_at')->where('final_exam_remark','FAIL')->count();
