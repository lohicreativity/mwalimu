...
class LoanAllocationController extends Controller
{
    ...
    {	
		$staff = User::find(Auth::user()->id)->staff;
		$ac_year = AcademicYear::where('id',$request->study_academic_year_id)->first();
        $headers = [
                      'Cache-Control'       => 'must-revalidate, post-check=0, pre-check=0',   
                      'Content-type'        => 'text/csv',
                      'Content-Disposition' => 'attachment; filename=Loans-Beneficiaries'.'- Yr-'.$request->year_of_study.'-'.$ac_year->year.'.csv',
                      'Expires'             => '0',
                      'Pragma'              => 'public'
              ];
