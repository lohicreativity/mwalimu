	  ...
	  */
	  public function downloadActiveStudents(Request $request)
	  {
		   ...
		   $students = Auth::user()->hasRole('hod')? Registration::whereHas('student.campusProgram.program.departments',function($query) use($staff){$query->where('id',$staff->department_id);})
                                                                 ...
                                                                 ->with(['student.campusProgram.program','student.disabilityStatus'])->where('study_academic_year_id',session('active_academic_year_id'))
                                                                 ->where('semester_id',session('active_semester_id'))->get() : 
                                                     Registration::whereHas('student.studentshipStatus',function($query){$query->where('name','ACTIVE');})->with(['student.campusProgram.program','student.disabilityStatus'])
