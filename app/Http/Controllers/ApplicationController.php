    ...
    public function applicantsRegistration(Request $request)
    {
         ...
         if($request->get('query')){
            $applicants = Applicant::whereDoesntHave('student')->whereHas('selections',function($query) use($request){
                 $query->where('status','SELECTED');
            })->with(['nextOfKin','intake','selections.campusProgram.program'])->where('campus_id',$staff->campus_id)->where('first_name','LIKE','%'.$request->get('query').'%')->orWhere('middle_name','LIKE','%'.$request->get('query').'%')->orWhere('surname','LIKE','%'.$request->get('query').'%')->where('application_window_id',$application_window->id)->where('confirmation_status','!=','CANCELLED')->where('confirmation_status','!=','TRANSFERED')->where('admission_confirmation_status','!=','NOT CONFIRMED')->where('status','ADMITTED')->get();
