...
class ApplicationController extends Controller
{
	 ...
	 */
	 public function resetSelections(Request $request)
	 {
		 ...
		 Applicant::where('application_window_id',$request->get('application_window_id'))->where('campus_id',$staff->campus_id)->where('program_level_ida',$request->get('program_level_id'))->where('status','!=','SELECTED')->orwhere('status','!=','ADMITTED')->where('status','!=','SUBMITTED')->update(['status'=>null]);
    ...
    public function registerApplicant(Request $request)
    {
        ...
        if(str_contains($applicant->nationality,'Tanzania')){
            ...
            $acpac->query("INSERT INTO invoices (INVNUMBER,INVDATE,INVDESC,IDCUST,NAMECUST,[LINENO],REVACT,REVDESC,REVREF,REVAMT,IMPORTED,IMPDATE) VALUES ('".$misc_invoice->control_no."','".date('Y',strtotime($misc_invoice->created_at))."','".$quality_assurance_fee->feeItem->feeType->description."','".$stud_reg."','".$stud_name."','1','".$quality_assurance_fee->feeItem->feeType->gl_code."','".$quality_assurance_fee->feeItem->feeType->name."','".$quality_assurance_fee->feeItem->feeType->description."','".($quality_assurance_fee->amount_in_usd*$usd_currency->factor)."','0','".date('Y',strtotime(now()))."')");
