...
class ApplicationController extends Controller
{
    ...
    public function showApplicantsList(Request $request)
    {
        ...
        if($request->get('application') == 'progress' && $request->get('duration') == 'today'){
           $applicants = Applicant::where('documents_complete_status',0)->where('submission_complete_status',0)->where('application_window_id',$request->get('application_window_id'))->whereDate('created_at','=',now()->format('Y-m-d'))->paginate(20);
        }elseif($request->get('application') == 'completed' && $request->get('duration') == 'today'){
           $applicants = Applicant::where('documents_complete_status',1)->where('submission_complete_status',0)->where('application_window_id',$request->get('application_window_id'))->whereDate('created_at','=',now()->format('Y-m-d'))->paginate(20);
    ...
    public function showDashboard(Request $request)
    {
         ...
         $data = [
            ...
            'total_applications'=>Applicant::where('application_window_id',$application_window->id)->where('campus_id',$application_window_id->campus_id)->count(),
