...
class ApplicationController extends Controller
{
    ...
    { return ApplicationBatch::whereIn('program_level_id',[1,2])->latest();
    	$data = [
           'awards'=>Award::all(),
           'intakes'=>Intake::all(),
    ...
    public function submitSelectedApplicants(Request $request)
    {
        ...
        if(count($applicants) > 0){
            ...
            if($current_batch->id == $batch_id){
                ...
                $new_batch->program_level_id = $award->id;
...
/*                 if(count($program->entryRequirements) == 0){
    ...
    public function selfSendAdmissionLetter(Request $request)
    {
        ...
        $program_fee = ProgramFee::select('amount_in_tzs','amount_in_usd')
                                 ->where('study_academic_year_id',$study_academic_year->id)
...
/*             if($request->get('app_'.$applicant->id) == $applicant->id){
    ...
    public function showInternalTransfer(Request $request)
    {
        ...
        $student = Student::whereHas('applicant',function($query) use($staff){$query->where('campus_id',$staff->campus_id);})
                          ->whereHas('academicStatus',function($query){$query->where('name','FRESHER');})
                          ->whereHas('studentshipStatus', function($query){$query->where('name', 'ACTIVE');})
                          ->with(['applicant.selections'=>function($query){$query->where('status','SELECTED');},'applicant.selections.campusProgram.program'])->where('registration_number',$request->get('registration_number'))->first();
    ...
    public function submitInternalTransfer(Request $request)
    {
        ...
        $ac_year = StudyAcademicYear::latest()->first();
		$semester = Semester::where('status','ACTIVE')->first();
        ...
        if($intake == 'March'){
		    $dates = SpecialDate::where('name','New Registration Period')->where('study_academic_year_id',$ac_year->id + 1)->where('campus_id',$applicant->campus_id)->where('intake',$intake)->get();
        }else{
    ...
    public function getVerifiedApplicantsNACTVET(Request $request)
    {
        ...
        if($no_of_applicants == 0){
            ...
            return redirect()->back()->with('message',$no_of_applicants.' verified applicants retrieved successfully from NACTVET');
        }
    }
