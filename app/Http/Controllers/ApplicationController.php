...
class ApplicationController extends Controller
{
    ...
    public function showApplicantsList(Request $request)
    {
        ...
        if($request->get('status') == 'progress'){
           $applicants = Applicant::with(['selections.campusProgram.program', 'programLevel'])->where('programs_complete_status',0)->where('submission_complete_status',0)->where('application_window_id',$request->get('application_window_id'))->where('campus_id',$application_window->campus_id)->paginate(20);
    ...
    public function downloadSelectedApplicants(Request $request)
    {
        ...
        $headers = [
         ...
         if($request->get('gender')){
            ...
            $list = Applicant::whereHas('intake.applicationWindows',function($query) use($request){
                 $query->where('id',$request->get('application_window_id'));
            })->with(['nextOfKin','intake', 'selections' => function ($query) { $query->where('status', 'APPROVING')->orWhere('status', 'SELECTED'); }, 'selections.campusProgram.program','nectaResultDetails.results','nacteResultDetails.results'])->where('program_level_id',$request->get('program_level_id'))->where('campus_id',$staff->campus_id)->get();
         }
