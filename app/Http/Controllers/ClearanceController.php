...
class ClearanceController extends Controller
{
    ...
    public function showList(Request $request)
    {
        if(Auth::user()->hasRole('hod')){
           $clearances = Clearance::with('student')->where('study_academic_year_id',$request->get('study_academic_year_id'))->whereNull('hod_status')->latest()->paginate(20);
        }elseif(Auth::user()->hasRole('finance-officer')){
           $clearances = Clearance::with('student')->where('study_academic_year_id',$request->get('study_academic_year_id'))->whereNull('finance_status')->latest()->paginate(20);
        }elseif(Auth::user()->hasRole('librarian')){
           $clearances = Clearance::with('student')->where('study_academic_year_id',$request->get('study_academic_year_id'))->whereNull('library_status')->latest()->paginate(20);
        }elseif(Auth::user()->hasRole('dean-of-students')){
           $clearances = Clearance::with('student')->where('study_academic_year_id',$request->get('study_academic_year_id'))->whereNull('hostel_status')->latest()->paginate(20);
        }else{
