...
/*             if(Util::stripSpacesUpper($semester->name) == Util::stripSpacesUpper('Semester 2')){
    ...
    public function getGlobalReport(Request $request)
    {
      ...
      $module_assignment = ModuleAssignment::whereHas('programModuleAssignment',function($query) use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'))->where('semester_id',1);})
