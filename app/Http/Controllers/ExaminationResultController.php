...
class ExaminationResultController extends Controller
{
    ...
    { 
      ...
      if($semester){
         ...
         foreach($enrolled_students as $student){
            ...
            if(str_contains($remark->remark,'IRREGULARITY')){
               ...
               $results = ExaminationResult::whereIn('module_assignment_id',$module_assignmentIDs)
                                             ...
                                             ->with(['retakeHistory'=>function($query) use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id') - 1);},
                                                     'retakeHistory.retakableResults'=>function($query) use($request){$query->latest();}])
                                             ->get();
               ...
               foreach($results as $result){
                  ...
                  $postponed_status = SpecialExam::where('study_academic_year_id',$request->get('study_academic_year_id'))
                                                   ->where('semester_id',$semester->id)
         ...
         $results = $processed_result = $grading_policy = $gpa_classes = $module_assignment_buffer = $optional_modules = null;
         ...
         if(Util::stripSpacesUpper($semester->name) == Util::stripSpacesUpper('Semester 2')){
            foreach($enrolled_students as $student){
               $annual_results = ExaminationResult::where('student_id',$student->id)
                                                ->whereHas('moduleAssignment.programModuleAssignment',function($query) use($module_assignments,$year_of_study){$query->where('campus_program_id',$module_assignments[0]->programModuleAssignment->campusProgram->id)
                                                                                                                                                                     ->where('year_of_study',$year_of_study);})
                                                ->with('moduleAssignment.programModuleAssignment')
                                                ->get();

               $student_annual_results_for_gpa_computation = [];
               foreach($annual_results as $result){
                  if($result->moduleAssignment->programModuleAssignment->category != 'OTHER'){
                     $student_annual_results_for_gpa_computation = $result;
                  }
               }

               $annual_credits = SemesterRemark::where('student_id',$student->id)
                                             ->where('study_academic_year_id',$request->get('study_academic_year_id'))
                                             ->where('year_of_study',$year_of_study)
                                             ->sum('credit');

               $annual_points = SemesterRemark::where('student_id',$student->id)
                                             ->where('study_academic_year_id',$request->get('study_academic_year_id'))
                                             ->where('year_of_study',$year_of_study)
                                             ->sum('point');

               $annual_semester_remarks = SemesterRemark::where('student_id',$student->id)
                                          ->where('study_academic_year_id',$request->get('study_academic_year_id'))
                                          ->where('year_of_study',$year_of_study)
                                          ->get();

               if($rm = AnnualRemark::where('student_id',$student->id)
                                    ->where('study_academic_year_id',$request->get('study_academic_year_id'))
                                    ->where('year_of_study',$year_of_study)->first()){
                  $annual_remark = $rm;
                  
               }else{
                  $annual_remark = new AnnualRemark;
               }

               $annual_remark->student_id = $student->id;
               $annual_remark->year_of_study = $year_of_study;
               $annual_remark->study_academic_year_id = $request->get('study_academic_year_id');
               $annual_remark->remark = Util::getAnnualRemark($annual_semester_remarks,$annual_results);

               if($annual_remark->remark == 'INCOMPLETE' || $annual_remark->remark == 'POSTPONED' || $annual_remark->remark == 'SUPP'){

                  if($annual_remark->remark == 'SUPP'){

                     $annual_remark->gpa = Util::computeGPA($annual_credits,$student_annual_results_for_gpa_computation,'Other');
                     if($annual_remark->gpa < 2.0){
                        $annual_remark->remark = 'FAIL&DISCO';
                        $annual_remark->point = $annual_points;
                        $annual_remark->credit = $annual_credits;
                     }else{
                        $annual_remark->gpa = null;
                     }
               ...
               $annual_gpa_class = GPAClassification::where('nta_level_id',$ntaLevel->id)
                                                   ->where('study_academic_year_id',$request->get('study_academic_year_id'))
                                                   ->where('min_gpa','<=',bcdiv($annual_remark->gpa,1,1))
                                                   ->where('max_gpa','>=',bcdiv($annual_remark->gpa,1,1))
                                                   ->first();

               if($annual_remark->gpa && $annual_gpa_class){
                  $annual_remark->class = $annual_gpa_class->name;
               }else{
                  $annual_remark->class = null;
               }

               $annual_remark->save();

               $annual_results = $annual_remark = $annual_semester_remarks = $annual_gpa_class = $annual_credits = $student_annual_results_for_gpa_computation = null;

               $overall_results = ExaminationResult::where('student_id',$student->id)
                                                   ->whereHas('moduleAssignment.programModuleAssignment',function($query) use($module_assignments){$query->where('campus_program_id',$module_assignments[0]->programModuleAssignment->campusProgram->id);})
                                                   ->with('moduleAssignment.programModuleAssignment')
                                                   ->get();
         
               $student_overall_results_for_gpa_computation = [];
               foreach($overall_results as $result){
                  if($result->moduleAssignment->programModuleAssignment->category != 'OTHER'){
                     $student_overall_results_for_gpa_computation = $result;
                  }
               }

               $overall_semester_remarks = SemesterRemark::where('student_id',$student->id)->get();

               $overall_credits = SemesterRemark::where('student_id',$student->id)
                                                ->sum('credit');

               $overall_points = SemesterRemark::where('student_id',$student->id)
                                             ->sum('point');

               if($rm = OverallRemark::where('student_id',$student->id)->first()){
                  $overall_remark = $rm;
               }else{
                  $overall_remark = new OverallRemark;
               }

               $overall_remark->student_id = $student->id;
               $overall_remark->remark = Util::getOverallRemark($overall_semester_remarks,$overall_results);

               if($overall_remark->remark == 'INCOMPLETE' || $overall_remark->remark == 'POSTPONED' || $overall_remark->remark == 'SUPP'){

                  if($overall_remark->remark == 'SUPP'){

                     $overall_remark->gpa = Util::computeGPA($overall_credits,$student_overall_results_for_gpa_computation,'Other');
                     if($overall_remark->gpa < 2.0){
                        $overall_remark->remark = 'FAIL&DISCO';
                        $overall_remark->point = $overall_points;
                        $overall_remark->credit = $overall_credits;
                     }else{
                        $overall_remark->gpa = null;
                     }
               ...
               $overall_gpa_class = GPAClassification::where('nta_level_id',$ntaLevel->id)
                                                   ->where('study_academic_year_id',$request->get('study_academic_year_id'))
                                                   ->where('min_gpa','<=',bcdiv($overall_remark->gpa,1,1))
                                                   ->where('max_gpa','>=',bcdiv($overall_remark->gpa,1,1))
                                                   ->first();

               if($overall_remark->gpa && $overall_gpa_class){
                  $overall_remark->class = $overall_gpa_class->name;
               }else{
                  $overall_remark->class = null;
               }

               $overall_remark->save();

               $academic_status_name = str_contains($overall_remark->remark,'POSTPONED Y') || str_contains($overall_remark->remark,'POSTPONED S')? 'POSTPONED' : $overall_remark->remark;
               $status = AcademicStatus::where('name',$academic_status_name)->first();
               $student = Student::find($student->id);
               $student->academic_status_id = $status->id;
               $student->save();
            }
         
            $overall_results = $overall_remark = $overall_semester_remarks = $overall_gpa_class = $overall_credits = $student_overall_results_for_gpa_computation = null;
         }
         ...
         $process->semester_id = $semester->id;
   ...
   public function processSuppResults(Request $request){
      ...
      $ntaLevel = $module_assignments[0]->programModuleAssignment->campusProgram->program->ntaLevel; // need to change it to final level name

      $supp_published = false;
      if(ResultPublication::whereHas('semester',function($query) use($semester_id){$query->where('id',$semester_id);})
                        ->where('study_academic_year_id',$ac_yr_id)
                        ->where('campus_id',$campus_program_id)
                        ->where('type','SUPP')
      ...
      $carry_cases = $carry_module_assignmentIDs = $carry_modules = $modules = $students = [];     
      if(!$supp_published){

         $previous_module_assignment = [];
         $carry_cases = ExaminationResult::select('student_id','module_assignment_id')->whereHas('moduleAssignment.programModuleAssignment',function($query) use($campus_program_id,$semester_id,$ac_yr_id){$query->where('campus_program_id',$campus_program_id)
                                          ...
                                          ->whereHas('student',function($query) use($campus_program_id){$query->where('campus_program_id',$campus_program_id);})
                                          ->whereHas('student.applicant',function($query) use($intake_id){$query->where('intake_id',$intake_id);})
                                          ->whereHas('student.studentshipStatus',function($query){$query->where('name','ACTIVE')->orWhere('name','RESUMED');})
                                          ->whereHas('student.registrations',function($query) use($semester_id,$ac_yr_id){$query->where('year_of_study',2)
                                                                                                                           ->where('semester_id',$semester_id)
                                                                                                                           ->where('study_academic_year_id',$ac_yr_id);})
         ...
         if(count($carry_cases) > 0){
            $previous_module_assignment = ModuleAssignment::whereHas('programModuleAssignment',function($query) use($campus_program_id,$semester_id,$ac_yr_id){$query->where('campus_program_id',$campus_program_id)
                                                                                                                                          ->where('year_of_study',1)
                                                                                                                                          ->where('semester_id',$semester_id)
                                                                                                                                          ->where('study_academic_year_id',$ac_yr_id - 1);})
                                                         ->get('id');
      ...
      $special_cases = SpecialExam::whereHas('student',function($query) use($campus_program_id){$query->where('campus_program_id',$campus_program_id);})
                                  ->whereHas('student.applicant',function($query) use($intake_id){$query->where('intake_id',$intake_id);})
                                  ->where('study_academic_year_id',$ac_yr_id)
                                  ->where('semester_id',$semester_id)
                                  ->where('type','FINAL')
      ...
      foreach($module_assignmentIDs as $assign_id){
         ...
         if($supp_published){
            ...
            $special_case_IDs = [];
            foreach($special_cases as $special_exam_case){

               if($special_exam_case->module_assignment_id == $assign_id){
                  $special_case_IDs[] = $special_exam_case->student_id;
               }
            }
            $cases = ExaminationResult::whereIn('student_id',$special_case_IDs)
                                      ...
                                      ->whereIn('supp_remark',['FAIL','INCOMPLETE'])
            ...
            $cases = ExaminationResult::whereHas('student.studentshipStatus',function($query){$query->where('name','ACTIVE')->OrWhere('name','RESUMED');})
                                      ->whereHas('student.semesterRemarks', function($query){$query->where('remark','SUPP')->orWhere('remark','INCOMPLETE')->orWhere('remark','CARRY')->orWhere('remark','RETAKE');})
                                      ->whereNotNull('final_uploaded_at')
         ...
         if($supp_published && count($cases) == 0){
            DB::rollback();
            return redirect()->back()->with('error','Cannot process published supplementary results'); 
         }

         $count = 0;
         $continue = null;
         foreach($cases as $case){
            if($case->supp_uploaded_at != null){
               $continue = true;
               break;
            }else{
      ...
      if($supp_published){
         ...
         $supp_cases = ExaminationResult::whereIn('student_id',$special_case_IDs)
                                        ->whereHas('student.studentshipStatus',function($query){$query->where('name','ACTIVE')->orWhere('name','RESUMED');})
                                        ->whereHas('student.registrations',function($query) use($year_of_study,$semester_id,$ac_yr_id){$query->where('year_of_study',$year_of_study)
                                                                                                                                             ->where('semester_id',$semester_id)
                                                                                                                                             ->where('study_academic_year_id',$ac_yr_id);})
                                        ->whereIn('supp_remark',['FAIL','INCOMPLETE'])
         ...
         $supp_cases = ExaminationResult::whereHas('student',function($query) use($campus_program_id){$query->where('campus_program_id',$campus_program_id);})
                                        ->whereHas('student.applicant',function($query) use($intake_id){$query->where('intake_id',$intake_id);})
                                        ->whereHas('student.studentshipStatus',function($query){$query->where('name','ACTIVE')->orWhere('name','RESUMED');})
                                        ->whereHas('student.registrations',function($query) use($year_of_study,$semester_id,$ac_yr_id){$query->where('year_of_study',$year_of_study)
                                                                                                                                             ->where('semester_id',$semester_id)
      ...
      foreach($students as $case){
         ...
         if(!empty($remark)){
            ...
            $remark->remark == 'REPEAT' || $remark->remark == 'FAIL&DISCO' || $remark->remark == 'DECEASED'){
               ...
               foreach($results as $result){
                  ...
                  if(count($special_exam_status) > 0 && $result->final_remark == 'POSTPONED' && !$supp_published){
         ...
         $supp_exams = $retake_exams = $carry_exams = [];
         foreach($student_results as $result){
            // if($result->student_id == 3559 && $result->module_assignment_id == 225){ return $result->supp_remark;}
         ...
         if(!empty($remark)){
            ...
            if(count($special_exam_status) > 0 && !$supp_published){
               $remark->remark = !empty($pass_status)? $pass_status : 'INCOMPLETE';
            }else{
      ...
      $publication->semester_id = $semester_id;
      $publication->type = 'SUPP';
      $publication->campus_id = $campus_id;
      $publication->nta_level_id = $ntaLevel->id;
      $publication->published_by_user_id = Auth::user()->id;
      $publication->save();
      
      $process = new ExaminationProcessRecord;
      $process->study_academic_year_id = $ac_yr_id;
      $process->semester_id = 0;
      $process->year_of_study = $year_of_study;
    ...
    { 
      try{
         ...
         if($res = ExaminationResult::where('module_assignment_id',$module_assignment->id)
            ...
            $result->course_work_score = $request->get('course_work_score');
            $score_before = $request->get('exam_type') == 'SUPP' || $result->final_remark == 'FAIL' && $result->supp_uploaded_at != null? $result->supp_score : $result->final_score;
            $result->final_score = $request->get('final_score');
            ...
            }elseif($result->supp_uploaded_at != null && $result->supp_score == null && $result->final_exam_remark == 'FAIL'){
               $result->supp_remark = 'INCOMPLETE';
               $result->supp_grade = 'I';
            }
      ...
      //  $request->get('module_assignment_id').'/'.
         if($request->get('supp_score') || $supp_edit_status){
         ...
         // if(is_null($request->get('process_type')) && Util::stripSpacesUpper($semester->name) == Util::stripSpacesUpper('Semester 1')){
            ...
            if(!str_contains($remark->remark,'IRREGULARITY')){
               ...
               if($remark->supp_remark != null || $request->get('process_type') == 'SUPP'){
                  DB::commit();
                  return redirect()->to('academic/results/process-student-supp-results?semester_id='.$semester->id.'&student_id='.$student->id.'&year_of_study='.$yr_of_study.'&ac_yr_id='.$ac_yr_id); 
               }
            }
    ...
    public function processStudentSuppResults(Request $request){
      ...
      foreach($module_assignments as $module_assignment){
         ...
         $module_assignment_buffer[$module_assignment->id]['course_work_based'] = $module_assignment->module->course_work_based;
         $module_assignment_buffer[$module_assignment->id]['final_pass_score'] = $module_assignment->module->final_pass_score;
         $module_assignment_buffer[$module_assignment->id]['course_work_pass_score'] = $module_assignment->module->course_work_pass_score;
      }
      ...
      $carry_case = ExaminationResult::select('student_id','module_assignment_id')->whereHas('moduleAssignment.programModuleAssignment',function($query) use($student,$semester_id,$ac_yr_id){$query->where('campus_program_id',$student->campus_program_id)
                                       ...
                                       ->whereHas('student.registrations',function($query) use($semester_id,$ac_yr_id){$query->where('year_of_study',2)
                                                                                                                        ->where('semester_id',$semester_id)
                                                                                                                        ->where('study_academic_year_id',$ac_yr_id);})
                                       ->whereHas('carryHistory',function($query) use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id') - 1);})
                                       ->where('student_id',$student->id)
                                       ->get();

      $previous_module_assignment = $carry_module_assignmentIDs = [];                              
      if(count($carry_case) > 0){
      ...
      foreach($previous_module_assignment as $assignment){
         ...
         $module_assignment_buffer[$assignment->id]['course_work_based'] = $assignment->module->course_work_based;
         $module_assignment_buffer[$assignment->id]['final_pass_score'] = $assignment->module->final_pass_score;
         $module_assignment_buffer[$assignment->id]['course_work_pass_score'] = $assignment->module->course_work_pass_score;
      }
      ...
      $module_assignments = $remark = null;
      if(count($carry_case) > 0){
         $remark = SemesterRemark::where('student_id',$student->id)
                                 ->where('study_academic_year_id',$ac_yr_id - 1)
                                 ->where('semester_id',$semester_id)
                                 ->where('year_of_study',1)
                                 ->first();
      ...
      if(!empty($remark)){
         if(!str_contains($remark->remark,'IRREGULARITY') && !str_contains($remark->remark,'POSTPONED Y') && !str_contains($remark->remark,'POSTPONED S')){
            if(count($carry_case) > 0){
               $results = ExaminationResult::where('student_id',$student->id)
                                          ->whereIn('module_assignment_id',$carry_module_assignmentIDs)
                                          ->get();
            ...
            foreach($results as $result){
               ...
               // }elseif($result->supp_remark != null && $result->final_remark != 'POSTPONED'){
                  ...
                  if($result->supp_score != null){
                     if($result->supp_score < $module_pass_mark){
                        ...
                        if($module_assignment->module->ntaLevel->id == 4 && $module_assignment->programModuleAssignment->year_of_study == 1){
                            $processed_result->supp_remark = 'CARRY';
                        }else{
                            $processed_result->supp_remark = 'RETAKE';
                        }
   
                     }else{

                           if($module_assignment->module->ntaLevel->id > 4){
                              $processed_result->supp_grade = 'B';
                              $processed_result->supp_point = 3;
   
                           }else{
                              $processed_result->supp_grade = 'C';
                              $processed_result->supp_point = 2;

                           }
                           $processed_result->supp_remark = 'PASS';
                     }
                     
                     if($processed_result->supp_remark == 'RETAKE'){
                        $no_of_failed_modules++;
                        if($retake = RetakeHistory::where('id',$result->retakable_id)->first()){
                           $history = $retake;
                        }else{
                     ...
                     if($processed_result->supp_remark == 'CARRY'){
                        $no_of_failed_modules++;
                        if($carry = CarryHistory::where('id',$result->retakable_id)->first()){
                           $history = $carry;
                        }else{
                  ...
                  //          $result->course_work_remark = 'INCOMPLETE';
                  //       }else{
                  //          $result->course_work_remark = $course_work_pass_score <= round($result->course_work_score) ? 'PASS' : 'FAIL';
                  //       }
      ...
      if(!empty($remark)){
         ...
         if($remark->supp_remark != 'PASS' && $remark->supp_remark != 'FAIL&DISCO' && $remark->supp_remark != 'REPEAT' && $remark->supp_remark != 'POSTPONED SEMESTER' && $remark->supp_remark != 'POSTPONED YEAR'){
            if(count($carry_exams) > 0){
               ...
               $remark->supp_serialized = serialize(['supp_exams'=>$supp_exams]);
            }
         }else{
...
/* 			whereHas('studentshipStatus',function($query){
               ...
               if($semester->id == 1){
                  $students = Student::whereHas('applicant',function($query) use($request){$query->where('intake_id',$request->get('intake_id'));})
                                     ->whereHas('registrations',function($query) use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'))
                                                                                                     ->where('year_of_study',explode('_',$request->get('campus_program_id'))[2]);})
                                     ->with(['semesterRemarks'=>function($query) use ($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'))->where('year_of_study',explode('_',$request->get('campus_program_id'))[2]);},
                                             ...
                                             'specialExams'=>function($query)use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'));},
                                             'examinationResults.changes',
                                             'examinationResults.moduleAssignment.specialExams',
                                             'applicant:id,program_level_id'])
                                     ->where('campus_program_id',$campus_program->id)
                                     ->get();
                  ...
                  $students = Student::whereHas('applicant',function($query) use($request){ $query->where('intake_id',$request->get('intake_id'));})
                                     ->whereHas('registrations',function($query) use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'))
                                                                                                     ->where('year_of_study',explode('_',$request->get('campus_program_id'))[2]);})
                                     ->whereHas('annualRemarks',function($query) use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'))
                                                                                                     ->where('year_of_study',explode('_',$request->get('campus_program_id'))[2]);})
                                     ->with(['semesterRemarks'=>function($query) use ($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'))->where('year_of_study',explode('_',$request->get('campus_program_id'))[2]);},
                                             'semesterRemarks.semester',
                                             'annualRemarks'=>function($query) use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'))->where('year_of_study',explode('_',$request->get('campus_program_id'))[2]);},
                                             'examinationResults'=>function($query) use($assignmentIds){$query->whereIn('module_assignment_id',$assignmentIds);},
                                             'specialExams'=>function($query)use($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'));},
                                             'examinationResults.changes',
                                             'examinationResults.moduleAssignment.specialExams',
        ...
        foreach($students as $key=>$student){
         ...
         $data = [
            ...
            'supp_students'=> Student::whereHas('applicant',function($query) use($request){$query->where('intake_id',$request->get('intake_id'));})
                                       ...
                                       ->whereDoesntHave('specialExams',function($query)use($request,$assignmentIds){$query->where('study_academic_year_id',$request->get('study_academic_year_id'))->where('status','APPROVED')->whereIn('module_assignment_id',$assignmentIds);})
                                       ->with(['semesterRemarks'=>function($query) use ($request){$query->where('study_academic_year_id',$request->get('study_academic_year_id'))->where('year_of_study',explode('_',$request->get('campus_program_id'))[2]);},
                                                'semesterRemarks.semester',
                                                'examinationResults',
                                                'examinationResults.changes','examinationResults.moduleAssignment.specialExams','applicant:id,program_level_id'])
               ...
               'special_exam_students'=>Student::whereHas('applicant',function($query) use($request){$query->where('intake_id',$request->get('intake_id'));})
                                                ...
                                                ->where('campus_program_id',$campus_program->id)
                                                ->get(),
               'special_exam_supp_students'=>$special_exam_supp_students
         ];
...
/*             if(Util::stripSpacesUpper($semester->name) == Util::stripSpacesUpper('Semester 2')){
            ...
            if($request->get('semester_id') == 'SUPPLEMENTARY'){
               ...
               if(count($special_exam_supp_students) == 0){
                  return redirect()->back()->with('error','No supplementary results for special examinations to be displayed');
               }
               return view('dashboard.academic.reports.final-program-results-supplementary-special',$data)->withTitle('Supplementary Program Results - '.$campus_program->program->name);
            }
