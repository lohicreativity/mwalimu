    ...
    public function store(Request $request)
    {
      ...
      if(Postponement::where('student_id',$request->get('student_id'))->where('study_academic_year_id',$request->get('study_academic_year_id'))->where('semester_id',$request->get('semester_id'))->where('status','!=','RESUMED')->count() != 0){
        ...
        if($request->get('category') == 'SEMESTER' || $request->get('category') == 'ANNUAL'){
            ...
            $final_result_upload_status = ModuleAssignment::whereHas('programModuleAssignment.campusProgram',function($query) use($student){$query->where('id',$student->campus_program_id);})
