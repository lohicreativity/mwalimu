...
//return $options;
    ...
    public function showStudentOverallResults(Request $request, $student_id, $ac_yr_id, $yr_of_study)
    {
         ...
         },'moduleAssignment','moduleAssignment.module','carryHistory.carrableResults'=>function($query) use($study_academic_year){$query->where('study_academic_year_id',$study_academic_year - 1);
         },'retakeHistory.retakableResults'=>function($query) use($study_academic_year){$query->where('study_academic_year_id',$study_academic_year - 1);}])->where('student_id',$student->id)->get();
