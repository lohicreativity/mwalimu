                      ...
                      @foreach($campus_program->programModuleAssignments as $assign)
                        ...
                        <td>{{ $assign->module->name }}
                          @if(count($assign->module->moduleAssignments) != 0)
                            ...
                            @endif
                            <p class="ss-font-xs ss-no-margin ss-italic">{{ $modAssign->staff->phone }}, {{ $modAssign->staff->email }} @if($modAssign->confirmed == 1) <span class="badge badge-success">Approved</span> @elseif($modAssign->confirmed == 0) <span class="badge badge-warning">Rejected</span> @else <span class="badge badge-warning">Pending Approval</span> @endif</p>
                            @endcan
