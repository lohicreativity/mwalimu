      ...
      <div class="container">
        ...
        <div class="row">
           <div class="col-md-12">
              ...
              <p>I am pleased to inform you that you have been selected to join the Academy for {{ $program_duration }} year pursuing <strong>{{ $program_name }}</strong> You are required to report at the Academy on Monday 25th October 2021 ready for registration and One Week for Orientation Programme that will commence on Monday 25th October 2021. Please note that all 1st year students are required to attend the Orientation Programme. Classes will commence immediately on 1st November 2021.</p>

              <p>Please observe the following instructions.</p>
              <ol>
<li>  That you should pay annual tuition fee of <strong>{{ $program_fee_words }} ({{ $currency }} {{ number_format($program_fee) }} ) only</strong> (accommodation exclusive)*. Fees may be paid either in one or two installments at the rate of 60%, equivalent to <strong>{{ $currency }} {{ number_format(0.6*$program_fee) }}</strong> (payable during the First Semester) and 40% equivalent to <strong>{{ $currency }} {{ number_format(0.4*$program_fee) }}</strong> (Second Semester). Each installment should be paid at the beginning of each Semester. Also you should pay compulsory fee of <strong>{{ $currency }} {{ number_format($medical_insurance_fee) }}</strong> for Medical Insurance (Control number to be obtained from NHIF website) as well as <strong>{{ $currency }} {{ number_format($nacte_quality_assurance_fee) }}</strong> for NACTE quality assurance fee, <strong>{{ $currency }} {{ number_format($practical_training_fee) }}</strong> for Practical Training, <strong>{{ $currency }} {{ number_format($students_union_fee) }}</strong> for students union, <strong>{{ $currency }} {{ number_format($caution_money_fee) }}</strong> Caution Money, <strong>{{ $currency }} {{ number_format($medical_examination_fee) }}</strong> for Medical Examination, <strong>{{ $currency }} {{ number_format($registration_fee) }}</strong> for Registration and <strong>{{ $currency }} {{ number_format($identity_card_fee) }}</strong> for Identity Card at the beginning of first semester once a year. Parents/Sponsors/Students are requested to pay the fees through the <strong>Online System+ available at the Academy Website www.mnma.ac.tz.</strong></li>
