              ...
              @if($check_selected_applicant)
                  ...
                  @elseif($applicant->confirmation_status != 'CANCELLED' && $applicant->status == 'SELECTED' && $check_selected_applicant->selections[0]->status == 'SELECTED')
                      <div class="alert alert-success">
                        ...
                        @if($applicant->multiple_admissions === null || $applicant->multiple_admissions === 0) Please @if($ready_for_admission) click <a href="{{ url('application/self-send-admission-letter?applicant_id='.$applicant->id) }}">here</a> to proceed with admission process or <a 
                        href="{{ url('application/admission-confirmation') }}">here</a> to cancel the admission.@else
                            wait for admission package or click <a href="{{ url('application/admission-confirmation') }}">here</a> to cancel the admission. @endif @elseif($applicant->confirmation_status === null) Please <a href="{{ url('application/admission-confirmation') }}">click here</a> to confirm with us.@endif</h3>
                      </div>
