									    ...
									    <table class="table table-bordered ss-paginated-table" style="font-size:10pt">
										  ...
										  <tbody>
											...
											<tr>
											   <td>{{ ($key+1) }}</td>
											   <td>{{ date('Y-m-d',strtotime($payments->created_at))}}</td>
											   <td title="Invoice#: {{ $payments->reference_no }}">{{ $payments->control_no }} @if($invoice && $payments->control_no == $invoice->control_no)<a href="{{ url('student/reset-control-number?control_no='.$payments->control_no) }}"> &nbsp; &nbsp; <span style="color: red; font-style:italic">Reset</a> @endif</td> 
											   <td>{{ $payments->feeType->name }}</td> 											   
											   <td>
												@if(str_contains($payments->feeType->name,'Tuition'))
													@if($tuition_fee_loan > 0)
